...
computeDailyBaseChargeReport = async (rawDataSet, params) =>{
    console.log('computeRequestCountReport');

    let monthNo, dayNo, week_from_date = null, month_from_date = null;
    let outerObj, hourlyBasisTotalCount = [], dayWiseTotalCount = [], weekWiseTotalCount = [], monthWiseTotalCount = [];
    let dataObj = {dailyBase: 0};
    let dayDataObj = {dailyBase: 0};
    let weeklyDataObj = {dailyBase: 0};
    let monthlyDataObj = {dailyBase: 0};
    if (rawDataSet.length > 0){
        for (let i=0; i<rawDataSet.length; i++){
            outerObj = rawDataSet[i];
            if (outerObj.baseCharge){

                dataObj.dailyBase = dataObj.dailyBase + outerObj.baseCharge;
                dayDataObj.dailyBase = dayDataObj.dailyBase + outerObj.baseCharge;
                weeklyDataObj.dailyBase = weeklyDataObj.dailyBase + outerObj.baseCharge;
                monthlyDataObj.dailyBase = monthlyDataObj.dailyBase + outerObj.baseCharge;

                // reset start_date for both month & week so can update with latest one
                if (week_from_date === null) week_from_date = outerObj.date;
                if (month_from_date === null) month_from_date = outerObj.date;
                ...
                if(Number(dayNo) === Number(helper.getDaysInMonth(monthNo))){
                    ...
                    monthlyDataObj = _.clone({dailyBase: 0});
                ...
                if (Number(dayNo) % 7 === 0){
                    ...
                    weeklyDataObj = _.clone({dailyBase: 0});
                    week_from_date = null;
                }
                ...
                dayDataObj = _.clone({dailyBase: 0});
            }
        }
...
module.exports = {
    computeDailyBaseChargeReport: computeDailyBaseChargeReport,
